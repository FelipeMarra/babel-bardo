...
from babel_bardo.eval_metrics import get_kld, get_kld_for_segments_transitions, get_kld_for_transitions, get_fad_vggish

RPGNAME = 'O Segredo Na Ilha'
...
for idx_v, video in enumerate(playlist):
    ...
    bardo_templates:list[BardoTemplate] = [
        Bardo1(RPGNAME, BARDO_ROOT_PATH, video_id, translate=True, language='pt'), #Bardo1 commes 1st because it don't use Ollama
        Bardo0(RPGNAME, BARDO_ROOT_PATH, video_id, language='pt'),
        Bardo2(RPGNAME, BARDO_ROOT_PATH, video_id, language='pt'),
        Bardo3(RPGNAME, BARDO_ROOT_PATH, video_id, language='pt')
    ]
