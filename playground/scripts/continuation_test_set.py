...
def set_tests() -> List[ContinuationTest]:
    ...
    test1 = ContinuationTest(
        [
            TestParams(duration=40, description=DESCRIPTION),
            TestParams(duration=40, extend_stride=10),
            TestParams(duration=40, extend_stride=10)
        ]
    )
